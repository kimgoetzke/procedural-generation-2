        ...
        if let Some(tile) = tile {
          if tile.tile_type == TileType::Single {
            if let Some(tile_below) = plane_below.get_tile(tile.coords.internal_grid) {
              ...
              // TODO: Find out why the below happening - it can conflict with the pathfinding logic for edge
              //  connection points when the flat plane tile below was assumed to be Fill (see next warning below)
              warn!(
