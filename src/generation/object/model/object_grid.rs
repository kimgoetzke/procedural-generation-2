...
use crate::generation::model::{Climate, LayeredPlane, TerrainType, TileType};
use crate::generation::object::model::cell::PropagationFailure;
use crate::generation::object::model::connection::get_connection_points;
use crate::generation::object::model::{Cell, Connection, ObjectGridSnapshot, TerrainState};
use bevy::log::*;
...
mod tests {
  ...
  fn reduce_neighbour_updates_cells_in_every_cardinal_direction() {
    ...
    reference.initialise(
      ...
      &[TerrainState::new_with_no_neighbours(
        crate::generation::object::model::ObjectName::Empty,
        0,
        1,
      )],
      vec![],
    ...
    *grid.get_cell_mut(&reference_ig).unwrap() = reference;
    ...
    for (connection, target_ig) in get_connection_points(&reference_ig) {
      ...
      target.initialise(
        ...
        TileType::Fill,
        &[
          TerrainState::new_with_no_neighbours(crate::generation::object::model::ObjectName::Empty, 0, 1),
          TerrainState::new_with_no_neighbours(crate::generation::object::model::ObjectName::Land1IndividualObject1, 1, 1),
        ],
        vec![],
        false,
      );
  ...
  fn restore_from_snapshot_only_restores_object_cells() {
    ...
    cell.mark_as_collapsed(crate::generation::object::model::ObjectName::PathTop);
